...
namespace SevenWondersGUI
{
    ...
    {               
        ...
        private bool wonderBoardExceptionResource = false;
        private bool wonderBoardExceptionManGood = false;
        ...
        */
        ...
        public void tradeTo(PlayerState p, int resourceIndex, int player){
            ...
            for (int i = 0; i < ComCards[p.getName()].Count; i++)
            {
                if ((resourceIndex > 3) && (p.getBoard().getName() == "WB10") && (p.getBoard().getCurrentWonderLevel() > 0)) { coinAmount = 1; }
                //Market Card
        ...
        public int[] SpecialResourceArray(PlayerState p) {
            ...
            {
                for (int j = 0; j < 7; j++)  {  specialResourceTable[j] += SResources[p.getName()][i].getResources()[j]; }
            }
